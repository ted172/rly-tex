...
class Document
  ...
  def initialize(rly_file)
    @sections = Array.new
    @document = recurse_insert_rly(rly_file)
    @document.join.split(/^\n/).each do |rly_text|
  ...
  def recurse_insert_rly(rly_file)
    inlined_rly = rly_file.class == String ? IO.readlines(rly_file) : rly_file
    inlined_rly.map do |line|
      if line =~ /^\\insert\s+(.*\.rly)/
        recurse_insert_rly(IO.readlines($1))
      else
        line
      end
    end
...
#==============================================================================
# Writer classes to convert RLY format to PDF, MSWord, LaTex, HTML
...
class HtmWriter
  def initialize(rly_file)
    ...
    htm << '<html>'
    htm << '<head>'
    htm << IO.read(File.dirname(__FILE__)+"/rly.css")
  ...
  def inlines(txt)
    ...
    mrk = $1
    ref = $2
    txt.gsub!(/\bP\{/, "") # P{pag_ref}
    txt.gsub!(/\bf\{/, "<a href=\"#{ref}\">") # f{fig_ref}
    txt.gsub!(/\bt\{/, "") # t{tab_ref}
    txt.gsub!(/\bs\{/, "") # s{sec_ref}
    txt.gsub!(/\bS\{/, "") # S{sec_ref}
    txt.gsub!(/\be\{/, "<em>")  # e{emphasized}
    txt.gsub!(/\bi\{/, "<i>")   # i{italic}
    txt.gsub!(/\bb\{/, "<b>")   # b{bold}
    txt.gsub!(/\bu\{/, "<ins>") # u{underline}
    txt.gsub!(/\bc\{/, "") # c{code}
    txt.gsub!(/\bF\{/, "") # F{footnote}
    if txt =~ /\bv\{/      # v{verbatim}
      txt.gsub!(/\bv\{/, "")
      txt.gsub!(/\}/, "")
    end
    ...
    case mrk
    when 'u' then txt.gsub!(/\}/, "</ins>")
    when 'b' then txt.gsub!(/\}/, "</b>")
    when 'i' then txt.gsub!(/\}/, "</i>")
    when 'e' then txt.gsub!(/\}/, "</em>")
    when 'f' then txt.gsub!(/\}/, "</a>")
    end
    return txt
  end
  def heading(hd)
  ...
  def tabular(tbl)
    htbl = []
    htbl << "<table style=\"width:100%\">"
    tbl.entries.each do |row_items|
      htbl << "<tr>"
      htbl << row_items.map{|e|"<td>#{inlines(e)}</td>"}.join
      htbl << "</tr>"
    end
    htbl << "</table>"
    return htbl.join("\n")
  ...
  def insert(ins)
    system "fig2dev -L svg #{ins.file} #{ins.path}.svg" if !File.exist?(ins.path+".svg")
    ins.ref = File.basename(ins.path).split(/_/).map{|e|e.capitalize}.join("")
    return "<img src=\"#{ins.path}.svg\" id=\"#{ins.ref}\" alt=\"#{ins.caption}\">\n"
  end
  def section(sec)
    return ""
  end
...
RLY.new(ARGV.dup) if $0 == __FILE__
